...
`docker_postgres_data_dir` must resolve inside the role-managed `docker_postgres_data_volume` without being shadowed by an anonymous volume declared by the base PostgreSQL image. Do not use `/var/lib/postgresql/data` while mounting the named volume at `/var/lib/postgresql`; use a versioned subdirectory such as `/var/lib/postgresql/18/docker`, or mount the named data volume directly at the configured `PGDATA` path.
