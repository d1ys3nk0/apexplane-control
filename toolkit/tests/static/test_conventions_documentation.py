...
from apc_target_static import TOOLKIT_ROOT, target_repo_root
...
def referenced_static_test_exists(reference: str) -> bool:
    ...
    base_root = TOOLKIT_ROOT.parent if path_text.startswith("toolkit/") else REPO_ROOT
    path = base_root / path_text
    if not path.is_file():
        return False
...
def test_automated_conventions_reference_existing_static_tests() -> None:
    ...
    for bullet in automated_bullets(text):
        ...
        if not references:
            ...
            continue
        errors.extend(
            f"referenced static test does not exist: {reference}"
